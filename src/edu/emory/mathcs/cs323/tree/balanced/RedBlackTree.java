...
public class RedBlackTree<T extends Comparable<T>> extends AbstractBalancedBinarySearchTree<T, RedBlackNode<T>> {
	...
	private void balanceWithBlackUncle(RedBlackNode<T> node) {
		/*
		 * Rotation cases (G = grandparent, P = parent, C = child) Case 1: G / P
		 * / C
		 * 
		 * Case 2: G / P \ C
		 * 
		 * Case 3: G \ P \ C Case 4: G \ P / C
		 */
		...
		if (grandParent != null) {
			...
			if (grandParent.isLeftChild(parent) && parent.isRightChild(node)) // Case
																				// 2
			{
				rotateLeft(parent);
				node = parent;
			} else if (grandParent.isRightChild(parent) && parent.isLeftChild(node)) // Case
																						// 4
			{
				rotateRight(parent);
				node = parent;
