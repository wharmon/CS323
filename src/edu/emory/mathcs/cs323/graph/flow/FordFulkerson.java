...
public class FordFulkerson extends MFAlgorithm {
	 ...
	 *            Graph
	 * @param sub
	 *            that contains both forward (and backward) edges
	 * @param mf
	 *            Found MaxFlow
	 * @param min
	 *            Found weight of edge with minimum weight within the path list
	 */
	 ...
	 *            Found MaxFlow
	 * @param path
	 *            Found path from source to target
	 ...
	 *            Graph
	 * @param mf
	 *            MaxFlow currently found
	 * @param sub
	 *            Subgraph that contains both forward (and backward) edges
	 * @param source
	 *            Source vertex
	 * @param target
	 *            Target vertex
	...
	private Subgraph getAugmentingPath(Graph graph, MaxFlow mf, Subgraph sub, int source, int target) {
		...
		for (Edge edge : graph.getIncomingEdges(target)) {
			if (sub.contains(edge.getSource()))
				continue; // cycle
			if (mf.getResidual(edge) <= 0)
