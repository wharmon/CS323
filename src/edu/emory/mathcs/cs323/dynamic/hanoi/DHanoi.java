...
public class DHanoi extends AbstractHanoi {
	...
	private void solve(List<String> list, int n, char source, char intermediate, char destination,
		...
		if (sub != null)
			addAll(list, sub[0], sub[1]);
		// Move all plates from 'source' to 'intermediate' via 'destination' as
		// medium
		else
			solve(list, n - 1, source, destination, intermediate, map);
		...
		if (sub != null)
			addAll(list, sub[0], sub[1]);
		// Move all plates from 'intermediate' to 'destination' via 'source' as
		// medium
		else
			solve(list, n - 1, intermediate, source, destination, map);
